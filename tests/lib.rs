...
fn check_basics() {
    ...
    assert_eq!(
        tinyexpr::interp("exp(1)").unwrap(),
        tinyexpr::interp("e").unwrap()
    );
    assert_eq!(tinyexpr::interp("floor(3.1415)").unwrap(), 3.0);
    assert_eq!(
        tinyexpr::interp("ceil(3.1415)*floor(3.1415)").unwrap(),
        12.0
    );
...
#[test]
fn check_constants() {
    assert_eq!(tinyexpr::interp("pi").unwrap(), std::f64::consts::PI);
    assert_eq!(tinyexpr::interp("e").unwrap(), std::f64::consts::E);
}
...
fn check_trigs() {
    ...
    assert_eq!(
        tinyexpr::interp("asin(1)").unwrap(),
        tinyexpr::interp("pi/2").unwrap()
    );
...
#[should_panic]
fn parse_error() {
    let _ = tinyexpr::interp("atan(foo)").unwrap_or_else(|e| {
        panic!("{}", e);
    });
}
